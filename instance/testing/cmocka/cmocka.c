                                           ...
                                           struct CMUnitTestState *cm_tests)
{
    ...
    print_message("[==========] %s: %zu test(s) run.\n",
                  group_name,
                  total_executed);
    print_error(GREEN "[  PASSED  ] %u test(s).\n" RESET,
                (unsigned)(total_passed));

    if (total_skipped) {
        print_error(YELLOW "[  SKIPPED ] %s: %zu test(s), listed below:\n" RESET,
                    group_name,
                    total_skipped);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
            ...
            if (cmtest->status == CM_TEST_SKIPPED) {
                print_error(YELLOW "[  SKIPPED ] %s\n" RESET, cmtest->test->name);
            }
        }
        print_error(YELLOW "\n %zu SKIPPED TEST(S)\n" RESET, total_skipped);
    }
    ...
    if (total_failed) {
        print_error(RED "[  FAILED  ] %s: %zu test(s), listed below:\n" RESET,
                    group_name,
                    total_failed);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
    ...
    /* One-line summary */
    print_error("\nSummary: [");
    for (int c = 0; c < (unsigned)total_passed; c++)
        print_error(GREEN "#" RESET);
    for (int c = 0; c < (unsigned)total_skipped; c++)
        print_error(YELLOW "?" RESET);
    for (int c = 0; c < (unsigned)(total_failed+total_errors); c++)
        print_error(RED "X" RESET);
    print_error("]\n\n");
}
                              ...
                              const char *error_message)
{
    ...
    case PRINTF_TEST_SUCCESS:
        print_message(GREEN "[       OK ] %s\n" RESET, test_name);
        break;
    case PRINTF_TEST_FAILURE:
        if (error_message != NULL) {
            print_error(RED "[  ERROR   ] --- %s\n" RESET, error_message);
        }
        print_message(RED "[  FAILED  ] %s\n" RESET, test_name);
        break;
    case PRINTF_TEST_SKIPPED:
        print_message(YELLOW "[  SKIPPED ] %s\n" RESET, test_name);
        break;
